...
PicoSite 提供 AOT 编译的单文件可执行程序，在 GitHub Actions 中：

- ⏱️ 构建耗时 **仅 1-5 秒**（同类工具需 30-120 秒）
- 💰 每月免费额度可跑 **数万次构建**
- 🔋 无需安装 .NET SDK、无需 restore/build，下载即用
...
· 想公开 → 写 /// 标准 XML 注释
· 不想公开 → 写 /* */ 或 // 普通注释
· PicoSite 只认 .xml 文件，其他注释编译器根本不生成

用户用“写什么注释”本身，就决定了“要不要出现在文档里”。零学习成本，纯天然 .NET 工作流。
